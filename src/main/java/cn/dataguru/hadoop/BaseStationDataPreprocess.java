...
import ethan.v16.ch03.CommonUtil;
...
public class BaseStationDataPreprocess extends Configured implements Tool {
	...
	public static class Map extends Mapper<LongWritable, Text, Text, Text> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			// 读取行
			try {
				tableLine.set(line, this.analysisPos, this.date, this.timepoint);
			} catch (LineException e) {
				if (e.getFlag() == -1)  //-1=时间不在参数指定的时间段内
					context.getCounter(Counter.OUTOFTIMESKIP).increment(1);
				else                    //0 时间格式有误
					context.getCounter(Counter.TIMESKIP).increment(1);
				return;
			} catch (Exception e) {    //源文件行有误
...
//			TODO 把counter写出去
			DataOutput out;
//			context.getCounter(Counter.LINESKIP).write();
		};
	}
	...
	public static class Reduce extends Reducer<Text, Text, NullWritable, Text> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			for (Text value : values) {
				...
				//^^ 每一次与基站通信，停留点都归结为上一个基站，停留时间都归结为上一次通信时间到今次通信时间
				//			的时间差
			try {
				// OFF基站是在所有key为（imsi：A，flag:09-17）的value计算完后额外增加D
				// 在最后添加“OFF”位置  ,最后OFF基站的通讯时间模拟为统计点的00分00秒
				...
				for (Entry<String, Float> entry : locs.entrySet()) {
					...
					builder.append(entry.getKey()).append("|");  //A
					builder.append(timeFlag).append("|");  // 09-17
					builder.append(entry.getValue());
//					0000000001|0000-0001|09-17|15.0
		...
		private HashMap<String, Float> getStayTime(TreeMap<Long, String> uploads) {
			...
			upload = it.next();   //第一个->第二个
			// 计算
			while (it.hasNext()) {
				nextUpload = it.next();  //第二个  //OFF
				float stay = (float) (nextUpload.getKey() - upload.getKey()) / 60.0f;
				if (stay <= 60.0) // 时间间隔过大则代表关机
				{
					if (locs.containsKey(upload.getValue()))  // 如果A基站有记录了，那值为前值+新值
						locs.put(upload.getValue(), locs.get(upload.getValue()) + stay);
					else
						locs.put(upload.getValue(), stay);
				}
				upload = nextUpload;  //^^ here,我不是为了能遍历到下一个，而是为了在下一次遍历中保留上一个的值
			}
			return locs;
		}
	}
...
//		args2[0] = "hdfs://u1:9000/home/hadoop/data.v16/NETWORK";
		args2[0] = "hdfs://u1:9000/home/hadoop/data.v16/POS.ch03";
		String output = args2[1] = "hdfs://u1:9000/home/hadoop/out.v16";
		args2[2] = "2013-09-12";
		args2[3] = "09-17-24";
		//保留三个最长停留基站
		...
		int res = ToolRunner.run(conf, new BaseStationDataPreprocess(), args2);
